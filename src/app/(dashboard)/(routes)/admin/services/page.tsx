  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <button onClick={()=>setShowForm(!showForm)} className="flex items-center gap-2 px-4 py-2 bg-violet-600 hover:bg-violet-500 text-white text-sm font-medium rounded-xl">
            ...
            {[
              ...
              <div key={field}>
                ...
                <input type={type} value={(form as any)[field]} onChange={e=>setForm({...form,[field]:e.target.value})} placeholder={placeholder}
                  className="w-full bg-white/5 border border-white/10 rounded-xl px-3 py-2.5 text-sm text-white placeholder-white/20 focus:outline-none focus:border-violet-500/40" required={field!=="upstreamId"} />
              </div>
            ))}
            <div>
            ...
            <div className="sm:col-span-2 flex items-center gap-6">
              {(["refill","cancel"] as const).map(f=>(
                <label key={f} className="flex items-center gap-2 cursor-pointer">
                  ...
                  <span className="text-sm text-white/60 capitalize">{f} enabled</span>
                </label>
              ))}
            </div>
            <div className="sm:col-span-2 flex gap-3">
              <button type="submit" className="px-5 py-2.5 bg-violet-600 hover:bg-violet-500 text-white text-sm font-medium rounded-xl">Create Service</button>
              <button type="button" onClick={()=>setShowForm(false)} className="px-5 py-2.5 border border-white/10 text-white/50 text-sm rounded-xl hover:border-white/20">Cancel</button>
            </div>
          </form>
        </div>
      )}
      ...
      <div className=" rounded-2xl overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-xs">
            ...
            <tbody>
              {loading ? <tr><td colSpan={7} className="px-4 py-10 text-center text-white/30">Loading...</td></tr>
              : services.map((s:any)=>(
                <tr key={s.id} className="border-t border-white/[0.03]">
                  <td className="px-4 py-3 text-white/80 max-w-[200px] truncate">{s.name}</td>
                  <td className="px-4 py-3 text-white/40">{s.category?.platform}</td>
                  <td className="px-4 py-3 text-emerald-400 font-mono">{formatNGN(s.rate)}</td>
                  <td className="px-4 py-3 text-white/40 font-mono">{s.minOrder.toLocaleString()}</td>
                  <td className="px-4 py-3 text-white/40 font-mono">{s.maxOrder.toLocaleString()}</td>
