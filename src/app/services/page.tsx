    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 pt-28 pb-20">
        <div className="text-center mb-12">
          ...
          <p className="text-white/40">Boost your presence across every major platform</p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-3 mb-8">
          <div className="relative flex-1">
            ...
            <input value={search} onChange={e=>setSearch(e.target.value)}
              className="w-full bg-white/5 border border-white/10 rounded-xl pl-10 pr-4 py-3 text-white placeholder-white/20 focus:outline-none focus:border-violet-500/40"
              placeholder="Search services..." />
          </div>
          <div className="relative">
            <select value={selectedPlatform} onChange={e=>setSelectedPlatform(e.target.value)}
              className="appearance-none bg-white/5 border border-white/10 rounded-xl px-4 py-3 pr-10 text-white/70 focus:outline-none focus:border-violet-500/40 cursor-pointer min-w-[160px]">
        ...
        <div className="surface-2 border-subtle rounded-2xl overflow-hidden">
          {loading ? (
            ...
            <div className="py-20 text-center text-white/30">No services found</div>
          ) : (
                ...
                <tbody>
                  {filtered.map((s:any,i:number)=>(
                    <tr key={s.id} className="border-t border-white/[0.03] hover:bg-white/[0.02]">
                      ...
                      <td className="px-4 py-3"><span className="px-2 py-0.5 bg-white/5 rounded text-white/50 text-xs">{s.category?.platform}</span></td>
                      <td className="px-4 py-3 text-emerald-400 font-mono font-medium">{formatNGN(s.rate)}</td>
                      <td className="px-4 py-3 text-white/40 font-mono text-xs">{s.minOrder.toLocaleString()}</td>
                      <td className="px-4 py-3 text-white/40 font-mono text-xs">{s.maxOrder.toLocaleString()}</td>
