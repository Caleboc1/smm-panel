      ...
      <div className="grid lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-4">
          <div className="flex gap-3">
            <div className="relative flex-1">
              ...
              <input value={search} onChange={e => setSearch(e.target.value)}
                className="w-full bg-white/5 border border-white/10 rounded-xl pl-10 pr-4 py-2.5 text-sm text-white placeholder-white/20 focus:outline-none focus:border-violet-500/40"
                placeholder="Search services..." />
            </div>
            <div className="relative">
              <select value={selectedCat} onChange={e => setSelectedCat(e.target.value)}
                className="appearance-none bg-white/5 border border-white/10 rounded-xl px-4 py-2.5 pr-8 text-sm text-white/70 focus:outline-none focus:border-violet-500/40 cursor-pointer">
          ...
          <div className="surface-2 border-subtle rounded-2xl overflow-hidden">
            {loading ? (
              <div className="py-16 text-center text-white/30">Loading services...</div>
            ) : filtered.length === 0 ? (
              <div className="py-16 text-center text-white/30">No services found</div>
            ) : (
                    ...
                    {filtered.map((s:any) => (
                      <tr key={s.id}
                        ...
                        <td className="px-4 py-3 text-white/80 max-w-[220px]">
                          ...
                          <div className="text-xs text-white/30 mt-0.5">{s.category?.platform}</div>
                        </td>
                        <td className="px-4 py-3 text-emerald-400 font-mono text-xs">{formatNGN(s.rate)}</td>
                        <td className="px-4 py-3 text-white/40 text-xs font-mono">{s.minOrder.toLocaleString()}</td>
                        <td className="px-4 py-3 text-white/40 text-xs font-mono">{s.maxOrder.toLocaleString()}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
        ...
        <div className="surface-2 border-subtle rounded-2xl p-6 h-fit sticky top-20">
          ...
          <form onSubmit={handleOrder} className="flex flex-col gap-4">
            <div>
              <label className="text-xs text-white/50 mb-1.5 block">Link / Username</label>
              <input value={link} onChange={e => setLink(e.target.value)} required disabled={!selected}
                className="w-full bg-white/5 border border-white/10 rounded-xl px-3 py-2.5 text-sm text-white placeholder-white/20 focus:outline-none focus:border-violet-500/40 disabled:opacity-40"
                placeholder="https://..." />
            </div>
            <div>
              <label className="text-xs text-white/50 mb-1.5 block">
                Quantity {selected && <span className="text-white/30">({selected.minOrder.toLocaleString()} – {selected.maxOrder.toLocaleString()})</span>}
              </label>
            ...
            {needsComment && (
              <div>
                <label className="text-xs text-white/50 mb-1.5 block">Custom Comments (one per line)</label>
                <textarea value={comment} onChange={e => setComment(e.target.value)} rows={3}
                  className="w-full bg-white/5 border border-white/10 rounded-xl px-3 py-2.5 text-sm text-white placeholder-white/20 focus:outline-none focus:border-violet-500/40 resize-none"
                  placeholder="Enter comments..." />
              </div>
            )}
            <div className="border-t border-white/5 pt-4 flex items-center justify-between">
              <span className="text-sm text-white/40">Total Charge</span>
